...
BEGIN
    ...
    SET THANHTIEN = 
        CASE 
            WHEN (SELECT COUNT(*) FROM PHIEUDANGKY P WHERE P.MAHOADON = H.MAHOADON) >= 20 
            THEN H.THANHTIEN * 0.85  
            ELSE H.THANHTIEN * 0.9   
        END
    FROM HOADON H
    JOIN INSERTED I ON H.MAHOADON = I.MAHOADON
    JOIN PHIEUDANGKY P ON I.MAHOADON = P.MAHOADON
    JOIN KHACHHANG K ON P.MAKHACHANG = K.MAKHACHHANG
...
BEGIN
    IF EXISTS (
        ...
        JOIN PHIEUDANGKY pd ON h.MAHOADON = pd.MAHOADON
        JOIN KHACHHANG k ON pd.MAKHACHANG = k.MAKHACHHANG
        WHERE k.LOAIKHACHHANG = N'Tự do' 
          AND EXISTS (SELECT 1 FROM GIAODICHCHUYENKHOAN g WHERE g.MATHANHTOAN = i.MATHANHTOAN)
    )
    BEGIN
        ROLLBACK TRANSACTION;
    END
...
BEGIN
    ...
    IF EXISTS (
        ...
        JOIN PHIEUDANGKY pd ON i.MAKHACHANG = pd.MAKHACHANG
